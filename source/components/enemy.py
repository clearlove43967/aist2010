...
def create_enemy(item, level):
    ...
    # GOOMBA
    if item['type'] == 0:
        goomba_color = item['goomba_color']
    else:
        color = item[c.COLOR]
    ...
    if item['type'] == c.ENEMY_TYPE_GOOMBA:
        sprite = Goomba(item['x'], item['y'], dir, goomba_color,
...
class Goomba(Enemy):
    ...
    def get_frame_rect(self, goomba_color):
        if goomba_color == c.DO:
            frame_rect_list = [(0, 0, 16, 16), (16, 0, 16, 16),
                               (32, 0, 16, 16)]
        elif goomba_color == c.RE:
